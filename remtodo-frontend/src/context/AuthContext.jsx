...
export function AuthProvider({ children }) {
  ...
  const login = async (email, password) => {
    try {
      ...
      // Fallback for offline / local mode if backend isn't started yet
      ...
      if (found) {
        setUser(found);
        localStorage.setItem('moosplanner_user', JSON.stringify(found));
      ...
      // If backend is completely offline, allow fallback session
      const fallbackUser = { id: Date.now(), displayName: email.split('@')[0], email, avatarConfig: {} };
  ...
  const signup = async (displayName, email, password) => {
    ...
    try {
      ...
      localStorage.setItem('moosplanner_local_users', JSON.stringify(localUsers));
